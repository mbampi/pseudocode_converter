...
public class Analysis {
    ...
    public String SyntacticAnalysis() throws VarDeclarationException, RWordNotFoundException, BadExpressionException {
        t = new Table();
        for (String file1 : file) {
            String[] line = file1.split("\\s+");//MODIFICADO: não suportava multiplos espaços.OBS:deve ser tratado para retirar espaços duplicados            if (line.length == 3) {
            if (":".equals(line[1])) {
            ...
            if ("escreva".equals(line[0].toLowerCase())) {
                this.testPrinting(line);
            }
            if ("leia".equals(line[0].toLowerCase())) {
